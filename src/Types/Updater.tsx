...
class Updater {
    ...
    public constructor(connectionPath: string, updatefunc= (msg: MessageEvent)=>{}, updatef=false) {
        this.broker = new Map<string, any>()
        this.name = getCookie("name")
        this.pid = getCookie("pid")

        this.handleWebsocketMessage = this.handleWebsocketMessage.bind(this)
        this.getNewResiliantWebsocket = this.getNewResiliantWebsocket.bind(this)

        if (updatef)
            this.ws = getNewResiliantWebsocket(`ws://${serverURL}${connectionPath}`, updatefunc)
        else
    ...
    public Request(data: any, type: string): Promise<any> {
        ...
        return new Promise<any>(async (resolve, reject) => {
            ...
            while (!gotResponse) {
                let response = self.broker.get(msgID)
    ...
    private send(data: any, type: string, id: string) {
        this.ws.send(JSON.stringify({ data, type, id, "pid": this.pid }))
    }
    ...
    private getNewResiliantWebsocket(wsConnectURL: string, onmessageFunc: any) {
        let websocket: WebSocket = new WebSocket(wsConnectURL)

        const retry = async () => {
            await new Promise(resolve => setTimeout(resolve, 2000))
            this.getNewResiliantWebsocket(wsConnectURL, onmessageFunc)
        }
        ...
        websocket.onopen = () => {
            ...
            if (self.name === "" || self.pid === "") {
                setCookie("name", name, 22)
                self.name = name
                setCookie("pid", pid, 22)
                self.pid = pid
            } else
                known = true
            ...
            if (!known)
                self.send(Object({ "name": name, "pid": pid}), "connect", this.getID())
        }
    }
}
